    ...
    <main className="flex-1 w-full pt-[200px] px-8 max-w-[1600px] mx-auto pb-24">
      ...
      <ul className="space-y-8 font-mono text-xl lg:text-3xl uppercase font-bold pr-4 max-w-5xl">
        {rules.map((rule, i) => (
          <li
            key={i}
            className="flex gap-6 items-start border-b-2 border-brand-black pb-4 hover:translate-x-4 transition-transform text-brand-black"
          >
            <span className="text-brand-white bg-brand-black px-2 py-1 text-sm rounded">
              0{i + 1}
            </span>
            {rule}
          </li>
